...
#define TRUE 1
#define FALSE 0
#define COPY_MODE 0644 //cp 源文件过大错误
...
const char* COMMAND_IN_NOT_COVER = "<<";
const char* COMMAND_OUT = ">";
const char* COMMAND_OUT_NOT_COVER = ">>";
...
enum {
	...
	ERROR_MISS_PARAMETER,//输入的命令参数不够
	ERROR_TOO_MANY_PARAMETER,//输入的命令参数太多
	ERROR_CD,//cd命令执行错误
	ERROR_CP,//cp命令执行错误
	ERROR_SYSTEM,
	ERROR_EXIT,
...
int callCp(int commandNum);
void oops(char * s1, char * s2);
int  in_fd, out_fd, n_chars, back_i, background_flag;
			...
			} else if (strcmp(commands[0], COMMAND_CP) == 0)
			{
				 ...
				 switch (result)
				 {
					...
					case ERROR_TOO_MANY_PARAMETER:
						fprintf(stderr, "\e[31;1mError: Too many parameters while using command \"%s\".\n\e[0m"
							, COMMAND_CP);
						break;
					case RESULT_NORMAL: // cp命令正常执行		 
					default:
						break;
				 }		
			}else { // 其它命令			
				/*查看命令行是否有后台运行符*/
				for (back_i=0; back_i < len; back_i++) {
					if (strncmp(commands[back_i], "&",1) == 0) {
						if (back_i == len-1) {
							background_flag = 1;
							break;
						}
				...
				switch (result) {
					...
					case ERROR_PIPE_MISS_PARAMETER:
						...
						break;
					case ERROR_CREATE:
					    fprintf(stderr, "\e[31;1mError: You have not  permission to create the files.\n\e[0m");
						break;
				}
			}
		}
...
int callCommandWithRedi(int left, int right) { // 所要执行的指令区间[left, right)，不含管道，可能含有重定向
	...
	/* 判断是否有重定向 */
	int inNum = 0, outNum = 0 , in_append_Num = 0, out_append_Num = 0;
	...
	for (int i=left; i<right; ++i) {
		if (strcmp(commands[i], COMMAND_IN) == 0) { // 输入重定向
			...
			if (endIdx == right) endIdx = i;
		}else if (strcmp(commands[i], COMMAND_OUT_NOT_COVER) == 0) { // 输出重定向,拼接不覆盖
			++out_append_Num;
			if (i+1 < right)
				outFile = commands[i+1];
			else return ERROR_MISS_PARAMETER; // 重定向符号后缺少文件名
	...
	}else if (out_append_Num > 1) { // 输出重定向符超过一个
		return ERROR_MANY_OUT;
	}
...
// int callCp(int commandNum) { // 执行cd命令
// 	int result = RESULT_NORMAL;
// 	if (commandNum < 3) {
// 		result = ERROR_MISS_PARAMETER;
...
// 		result = ERROR_TOO_MANY_PARAMETER;
// 	} else {
// 		fstream fs(commands[1],ios::in|ios::binary); //argv[1] 原文件
// 		fstream fs1(commands[2],ios::out|ios::binary); //argv[2] 目标文件
// 		char buf[1024] = {};
...
int callCp(int commandNum) { // 执行cd命令
	...
	{
		int fd=open(commands[1],O_RDWR|O_CREAT,S_IRWXU);//打开要拷贝的文件
		if(fd<0)
		{
			result = ERROR_MISS_PARAMETER;
			return 0;
		}
		int fd2=open(commands[2],O_RDWR|O_CREAT|O_EXCL,S_IRWXU);//建立一个新的空白文件
		if(fd2<0)
		{
			puts("the files have existed, now it will be coverred");
			fd2=open(commands[2],O_RDWR|O_CREAT,S_IRWXU);
			if(fd2<0)
			{
